...
def evaluate_summary(predicted_summary, user_summary, video_name, pred_score, eval_data):
    ...
    """
    y_pred2 = _to_numpy(predicted_summary)
    y_true2 = _to_numpy(user_summary)
    if y_true2.ndim > 1:
        y_true2 = y_true2.mean(axis=0)

    if eval_data == 'summe':
        curr_rho_coeff = stats.spearmanr(y_pred2, y_true2)[0]
        curr_tau_coeff = stats.kendalltau(stats.rankdata(-y_pred2), stats.rankdata(-y_true2))[0]
        ...
        score_np = _to_numpy(pred_score)
        tmp_rho_coeff = []
        tmp_tau_coeff = []
        for annotation in range(len(curr_user_score)):
            true_user_score = curr_user_score[annotation]
            curr_rho_coeff = stats.spearmanr(score_np, true_user_score)[0]
            curr_tau_coeff = stats.kendalltau(stats.rankdata(-score_np), stats.rankdata(-np.array(true_user_score)))[0]
