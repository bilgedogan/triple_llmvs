...
class SinusoidalTemporalPE(nn.Module):
    ...
    def _make_pe(self, T, device):
        position = torch.arange(T, device=device).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, self.input_dim, 2, device=device) * (-math.log(10000.0) / self.input_dim))
        pe = torch.zeros(1, T, self.input_dim, device=device)
        pe[0, :, 0::2] = torch.sin(position * div_term)
        pe[0, :, 1::2] = torch.cos(position * div_term)
        return pe
